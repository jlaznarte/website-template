jQuery(function($) {
  ...
  /**
   * Position du menu du full html
   */
  $(document).ready(function() {
    var yMain = $('#main').position();
    var yTop = yMain.top + 35;
    $('#bloc').css('top', yTop);
    $(window).scroll(function() {
      if ($(document).scrollTop() > yMain.top) {
        $('#bloc').css('top', '10px');
      } else {
        var hauteur = yMain.top;
        $('#bloc').css('top', yTop - $(document).scrollTop());
      }
    });
  });
