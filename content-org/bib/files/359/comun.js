...
var css_multimedia = "http://ep00.epimg.net/estilos/multimedia/multimedia.css";
...
var UrlPublicidad = "http://pubads.g.doubleclick.net/gampad/ads?sz=640x480&iu=%2F7811748%2Felpais_web%2Fplayer_video%2Fflash&ciu_szs=&impl=s&gdfp_req=1&env=vp&output=vast&correlator={random}&unviewed_position_start=1";
...
var UrlPublicidadCnbc = "http://pubads.g.doubleclick.net/gampad/ads?sz=640x480&iu=%2F7811748%2Felpais_cnbc_player_web%2Fflash&ciu_szs=&impl=s&gdfp_req=1&env=vp&output=vast&correlator={random}&unviewed_position_start=1";
...
if (event != "")
{
var evars_value = this._datosMarcado.media + "|" + this._datosMarcado.titulo.replace(/'|"|\|/g, "") + " - " + this._datosMarcado.referencia + "|" + (this._datosMarcado.playerName ? this._datosMarcado.playerName.replace(/'|"|\|/g, ""):"") + "|" + Math.floor(this._datosMarcado.duracion) + "|" + this._datosMarcado.tipoReproduccion + "|" + this._datosMarcado.publicidad + (this._datosMarcado.ad != "" ? " " + this._datosMarcado.ad : "" ) + "|" + this._datosMarcado.adCue + "|" + this._datosMarcado.agencia;
var newevars = evars;
...
launchAjaxOMN(newevars, evars_value, event,this._datosMarcado.tags);
}
}
}
...
{
document.getElementById(MultimediaPEP[MultimediaActual].id).innerHTML = "";
...
'<a class="MPEP-prox-btn-sig" href="javascript:void(0)" onclick="javascript:cargaVideo(false)"><span>Siguiente</span></a>'+
'</div>'+
'<span class="MPEP-prox-cont">Siguiente vídeo en <strong id="siguientes_contador">5</strong> segundos</span>'+
'<div class="MPEP-prox-video-media"><a href="javascript:void(0)" onclick="javascript:cargaVideo(false)"><img src="#foto#" class="MPEP-prox-video-img" alt="" /></a></div>'+
'<div class="MPEP-prox-video-txt">'+
'<div class="MPEP-prox-tit"><a href="javascript:void(0)" onclick="javascript:cargaVideo(false)">#titulo#</a></div>'+
'</div>'+
'</div>'+
'</div>'+
'</div></div>';
...
script.src = "http://ep00.epimg.net/rss/tags/continuo/c_" + fuente + ".js2";
...
var source = ref_datos[SiguientesVideosPEP._id].source;
var dims;
cargaVideo = function(pause) {
clearTimeout(SiguientesVideosPEP.cuenta_atras);
...
return new EPET_Video(SiguientesVideosPEP._datos, SiguientesVideosPEP._id, true);
};
...
{ 
if (i > 0 && (SiguientesVideosPEP.videoWallData[i].idRefBrightcove + UN) == id_video)
{
...
SiguientesVideosPEP.videoWallData[i].agrsNombreNormalizado = source;
if (i != 0) {
...
var video = cargaVideo(false);
if (video.tipo() == "html5" && SiguientesVideosPEP._datos.autoplay )
video.play();
};
SiguientesVideosPEP._datos = JSON.parse(JSON.stringify(SiguientesVideosPEP.videoWallData[ref_datos[SiguientesVideosPEP._id].idx]));
...
cod_img = '<img src="/t.gif" border="0" width="' + _datosVideo.anchoPlayer + '" height="' + _datosVideo.altoPlayer +'" id="' + _imgCaratula + '" onload="lzld(this)" data-src="' + _datosVideo.playerEPETParams["URLMediaStill"] + '" style="height:' + _datosVideo.altoPlayer + 'px" />';
else
cod_img = '<img src="' + _datosVideo.playerEPETParams[" border="0" width="' + _datosVideo.anchoPlayer + '" height="' + _datosVideo.altoPlayer +'" id="' + _imgCaratula + '"URLMediaStill"] + '" style="height:' + _datosVideo.altoPlayer + 'px" />';
...
keywords = "&cust_params=pbskey%3D" + escape(keywords) + "%26pos%3D{cuePointType}";
...
{
_marcado._datosMarcado.tags = _datosVideo.tagsIds;
}
if (_datosVideo.agrsIds != undefined && _datosVideo.agrsIds != "")
{
if (_marcado._datosMarcado.tags != "")
_marcado._datosMarcado.tags += ',';
_marcado._datosMarcado.tags += _datosVideo.agrsIds;
}
if ( primero_flash)
{
_HTML5vsFLASH = "flash";
...
this.controles.style.left = (this.width / 2 - 640 / 2) + "px";
}
else
{
...
barra.style.width = (this.controles.offsetWidth - this.ancho_controles) + "px";
...
var htmlCapaCentral = '<img src="/t.gif" id="imagen_contenedor" width="" height="" alt="' + LANG_MSG[LANG].foto + '"/><p id="pie_contenedor" class="figcaption estirar"></p>';
...
if (fotoUsuario != '' ) {
contenido_html = contenido_html + '<div class="foto"><img src="' + fotoUsuario + '" width="100" height="100" alt="' + LANG_MSG[LANG].avatar + '"></div><!-- === .foto === -->';
}
