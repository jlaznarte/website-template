...
function searchPageName() {
	if (jQuery("#scTabs li:nth-child(1).selected").length > 0) {
		s.pageName="Search - document";
	}
	else if (jQuery("#scTabs li:nth-child(2).selected").length > 0) {
		s.pageName="Search - author";
	}
	else if (jQuery("#scTabs li:nth-child(3).selected").length > 0) {
		s.pageName="Search - affiliation";
	}
	else if (jQuery("#scTabs li:nth-child(4).selected").length > 0) {
		s.pageName="Search - advanced";
	}
	else {
		s.pageName="Search - unknown";
	}
...
function homePageName() {
	if (jQuery("#nonip_container").length == 0) {
		searchPageName();
	}
	else {
		s.pageName="Home";
...
jQuery.fn.parseTextToObject=function(d) {
	...
	jQuery.each(this,function(i,e) {
		var text=e.textContent.split(d);
...
p:[{t:'c',c:'homePageName();'}]},
{p:[{t:'c',c:'var actionPattern=new RegExp("\\/*.url");\n\nif (! actionPattern.test(window.location.href)) {\n	homePageName();	\n}'}]},
{x:{p:["/search/form.url"]},
...
{p:[{t:'c',c:'/*\nThis code identifies a CARS page through the presence of a <div>\ntag (shouldn\'t do this; need better solution long term).\n*/\n\nif (document.getElementById("standardCARSTemplatePage")) {\n	//var title = document.title;\n	//var myTitle = title.substring(title.lastIndexOf("-") + 1);\n	//s.pageName = jQuery.trim(myTitle);\n	s.pageName="CARS";\n	s.prop4="cars";\n	s.channel="cars";\n}'}]}];
s_tc_Scopus.cr();