...
* Plugin: on2 v1.0
*
* jQuery plugin to handle issues with static links not propagating for delegated events
*/
...
* Author: Elsevier
* Date: 06/21/2013
* Plugin:eventHandlers v1.0
*
* Handlers for different types of events
*/
s.eventHandlers=new function() {
	...
	var getClickSection=function(e) {
		...
		var t=$(e.target);
		var pi=t.parents("[id]");
		var pc=t.parent().closest("[class]");
		
		if(pi.length>2) section.push(pi[pi.length-2].id);
		else if(pi.length>1) section.push(pi[pi.length-1].id);
		else section.push("unknown");

		if(pi.length>0) section.push(pi[0].id);
		else section.push("unknown");

		section.push(t[0].id||pc[0].className||t[0].localName||"unknown");
		
		return section.join(":");
	}

	this.basicClickEvent=function(e) {	
	...
	this.contentViewEvent=function(e) {
		var s_save=$.extend({},s);
		...
		s.eVar52=getZone(e.target.href);
		switch(e.data) {
			case "pdf": s.events+=",event30";break;
			case "epub": s.events+=",event31";break;
			case "mobi": s.events+=",event32";break;
		}
		s.t();
...
if (s_tc_sciencedirect.cp({h:["sciencedirect.com"]})) {
$.expr[':'].pdf = function(e) {
...
$("#epubLink").on("click",null,"epub",s.eventHandlers.contentViewEvent);
$("#mobiLink").on("click",null,"mobi",s.eventHandlers.contentViewEvent);
$(document).on2("click","a:pdf","pdf",s.eventHandlers.contentViewEvent);
$(document).on2("click","a[href],[onclick],input,button",null,s.eventHandlers.basicClickEvent);
}
