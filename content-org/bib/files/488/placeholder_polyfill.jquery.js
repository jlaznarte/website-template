...
(function($) {
	// create ':focus' selector, only appears in jQuery 1.6+
    ...
    function positionPlaceholder(placeholder,input){
        ...
        // in case of a datepicker with class 'to' for date range, the left positioning will be increased +16
        if(input.hasClass('datepicker') && input.hasClass('to')){
        	placeholder.css({
                width : input.innerWidth()-(ta ? 20 : 4),
                height : input.innerHeight()-6,
                left : input.position().left+16,
    ...
    $.fn.placeHolder = function(config) {
        ...
        return $(this).each(function(index) {
            ...
            placeholder = $('<span>').addClass(o.options.className).text(text).appendTo(input.parent());
