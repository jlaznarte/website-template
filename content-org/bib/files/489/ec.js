...
corporate={ // scripts used in the banner
	...
	run				:function(){var b=doc.getElementsByTagName('body');if(b.length==1){var c=b[0].className;b[0].className=(c)?"js "+c:"js";}corporate.getDocLang();corporate.langSelector();corporate.minMaxCSS();tools.init();
		...
		corporate.include("/wel/surveys/wr_survey01/wr_survey.js",function(){
	...
	include			:function(srcFile,callback,ext){if(!corporate.isLoad){corporate.isLoad={};}var i,s=srcFile,f=callback,t=(corporate.isLoad[s])?true:false,j,e,h,r,doc=document;if(t==false){e=(ext)?ext:corporate.getExtention(s);if(e=="js"){i=doc.createElement('script');i.setAttribute('type','text/javascript');i.setAttribute('src',s);h=doc.getElementsByTagName('body')[0];}else if(e=="css"){i=doc.createElement('link');i.setAttribute('type','text/css');i.setAttribute('rel','stylesheet');i.setAttribute('media','all');i.setAttribute('href',s);h=doc.getElementsByTagName('head')[0];}if(typeof f=="function"){if(isIE){i.onreadystatechange=function(){j=this.readyState;if(j=="loaded"||j=="complete"){f(i);}};}else{i.onload=f;}}if(h){h.appendChild(i);}corporate.isLoad[s]=i;}else if(typeof f=="function"){f();}}
},
