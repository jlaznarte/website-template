...
this.s_eventHandlers=new function() {
	...
	this.customClick=function(e) {
		...
		if (getClickSection == "article:rightPane:relArtList:icon_pdf:RecommendedArticlePDFClickEvent"){
			s.eVar69-"article:rightPane:recommendedArticles";
...
//count PDF King pop-up displayed
var scPDFKingWaitCounter=1000;
var scPDFKingWaitTimer=setInterval(countPDFKingPopUp, 2000);
        function countPDFKingPopUp() {
		var pdfKingDisplay = $('#pdfModalWindow').css("display");
		var pdfBlock = "block";
			if (pdfKingDisplay == pdfBlock){
				s.events = "event44"; //count PDF King pop-up displayed
				s.linkTrackVars="events";
				s.linkTrackEvents=s.events;
				s.tl(true,'o','PDFKing Popups');
				s.events=null;
				s.linkTrackEvents=null;
				s.linkTrackVars=null;
				clearInterval(scPDFKingWaitTimer);}
				}

//capture click on PDF Download button in PDF King			
	$(document).on2( 'click' , "#multiPdfIcon","",function () {
		s.events = "event45";
		s.linkTrackVars="events";
		s.linkTrackEvents=s.events;
		s.tl(true,'o','PDFKing PDF Download');
		s.events=null;
		s.linkTrackEvents=null;
		s.linkTrackVars=null;
		});
...
//count new format created for PDF King DDM
	$(document).on2( 'click' , ".saveName","",function () {
		s.events = "event47";
		s.linkTrackVars="events";
		s.linkTrackEvents=s.events;
		s.tl(true,'o','PDFKing New Format');
		s.events=null;
		s.linkTrackEvents=null;
		s.linkTrackVars=null;
		});		
...
//capture format download and click of download button for PDF King DDM
	$(document).on2( 'click' , "#ddm","",function () {
		s.eVar68=$('#naming').find(":selected").text();
		s.events="event46";
		s.linkTrackVars="events,eVar68";
		s.linkTrackEvents=s.events;
		s.tl(true,'o','PDFKing PDF DDM Download');
		s.events=null;
		s.eVar68=null;
		s.linkTrackEvents=null;
		s.linkTrackVars=null;
