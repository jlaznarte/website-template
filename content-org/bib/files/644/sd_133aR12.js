...
var DynamicArtContent = {
  ...
  buildPdfExcerptHtml: function(scope) {
    ...
    $('.excerptLink,.excerptLink1').bind('click', function(e) { var nw = window.open($('.excerptLink').attr('href'), '_blank', 'height=' + $(window).height()*.9 +',width=' + $(window).width()*.9+',resizeable=yes,left=50,top=50'); e.preventDefault(); e.stopPropagation();});
...
//    $('#pStretcher').remove();
  ...
  processHash: function() {
    ...
    if (hash != 'undefined' && hash !='') {
      ...
      if(hash == "MMCvFirst" && $('dl.ecomponent').length ) {
        hash = $('dl.ecomponent').first().attr('id');
        if(typeof hash == 'undfined') {
...
/*
    //find first reference fragment
    var refFragId = -1;
    for(var i=0; i<Fragment.mapReferences.length; i++) {
      var fid = Fragment.getIntraRefFragId(Fragment.mapReferences[i]);
      if(fid) {
        fid = (fid.split('_')[1])*1;
     ...
     submitCitation:function(e,obj) {
      ...
      setPaneWidths: function () {
        ...
        if(pgLayout.showRightBar==false && $('#rightInner').css('display')!='none' && $('#rightPane').width()>20) {
          ...
          $('#innerPaddingFixedBox').css('width',pgLayout.rcFloatWidth-22);
          ...
          $('#innerPaddingFixedBox').css('width',pgLayout.rcWidth-2);
        ...
        $('#rightOuter .innerPaddingFixedBox dd.oaUserLicense').width($('#rightOuter .innerPaddingFixedBox dd.oaUserLicense').parent().width()*.9);
      },
...
$(document).ready(function() {
    ...
    if(SDU.isIE7() && SDM.IE7_User_Msg)$('#ieWarningMsg').ieWarning();
        //loadBreadCrumbs();
...
function updateCitedByCounts(citedByCounts,start,count) {
      ...
      .success(function(res) {
        ...
        if (refMap != null) {
          ...
          if(citedBySCEids != null && citedBySCEids != ""){
            updateCitedByCounts(citedBySCEids,start,count);
          }
        }
      })
      .error(function() {
...
var pdfCite = {
  ...
  openPDF:function(url, event) {
    ...
    if ($.browser.msie) {
       pdfWin=window.open('','_blank','width='+newWidth+',height='+newHeight+',resizable=yes,left=50,top=50');
       pdfWin.location.href (url);
    } else {
       pdfWin=window.open(url,'_blank','width='+newWidth+',height='+newHeight+',resizable=yes,left=50,top=50');
    }
    pdfWin.focus();
...
var rightSidePane={
  ...
  getBibliographicPanel:function(obj){
    ...
    var nonSerialInfo = '';
    if (SDM.editorsInChief)  { nonSerialInfo = decodeURIComponent(SDM.editorsInChief); }
            ...
            if(conceptID!=undefined || refCounter>trd) {
                ...
                var getArticleList=function(res){
                    if(res.TYPE!=undefined && res.TYPE!='' && res.citation.length>0){
                        ...
                        $('#mrwRefArtList').parent().after('<div class="gadSep divider "></div>');
                    }else{
